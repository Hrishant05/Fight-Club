...
class Player {
  ...
  heal (player) {
    ...
    return `${player.name} heals for ${healAmount} health`
  }
}
...
let player1 = new Player("Pikachu", 100, 10)
let player2 = new Player("Charizard", 100, 10)
...
document.addEventListener('keydown', function(e) {
  ...
  if(e.key =="a" && p2.health>0 && p1.health<100 && game.isOver == false ){
    p1.heal(p1)
...
document.addEventListener('keydown', function(e) {
  ...
  if(e.key =="l" && p1.health>0 && p2.health<100 && game.isOver == false ){
    p2.heal(p2)
